...
async function generateIcons() {
  ...
  await logo.resize(32, 32).png().toFile(path.join(publicDir, "favicon.png"));
  ...
  // 4. Tauri Icons
  // 32x32.png
  await logo.resize(32, 32).png().toFile(path.join(iconsDir, "32x32.png"));
  console.log("Created src-tauri/icons/32x32.png");

  // 128x128.png
  await logo.resize(128, 128).png().toFile(path.join(iconsDir, "128x128.png"));
  ...
  await logo.resize(512, 512).png().toFile(path.join(iconsDir, "icon.png"));
  ...
  await logo.resize(512, 512).png().toFile(path.join(iconsDir, "icon.icns"));
  ...
  for (const { name, size } of squareSizes) {
    await logo.resize(size, size).png().toFile(path.join(iconsDir, name));
