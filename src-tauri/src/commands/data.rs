...
pub fn reset_data(
    ...
    crate::log_command!("reset_data", {
        ...
        conn.execute("DELETE FROM circulation_logs", [])
            .map_err(|e| e.to_string())?;
        conn.execute("DELETE FROM circulations", [])
            .map_err(|e| e.to_string())?;
        ...
        if !keep_settings {
            conn.execute("DELETE FROM daily_summary_settings", [])
                .map_err(|e| e.to_string())?;
            conn.execute("DELETE FROM notification_plugins", [])
                .map_err(|e| e.to_string())?;
        }
