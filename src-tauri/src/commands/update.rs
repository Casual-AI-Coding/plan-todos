...
async fn read_last_check(app: &AppHandle) -> Result<Option<DateTime<Utc>>, String> {
    ...
    let content = fs::read_to_string(&path)
        .map_err(|e| format!("Failed to read last check file: {}", e))?;
...
async fn write_last_check(app: &AppHandle) -> Result<(), String> {
    ...
    fs::write(&path, content)
        .map_err(|e| format!("Failed to write last check file: {}", e))?;
...
pub async fn check_for_updates(app: AppHandle) -> Result<Option<UpdateInfo>, String> {
    ...
    if has_update {
        ...
        log::info!("No update available - current version {} is latest", current_version_str);
        Ok(None)
    }
}
...
pub async fn skip_version(app: AppHandle, version: String) -> Result<(), String> {
    ...
    fs::write(&path, content)
        .map_err(|e| format!("Failed to write skip version file: {}", e))?;
