...
describe("PlanForm", () => {
  ...
  describe("新建时 reminder_times 默认行为", () => {
    ...
    it("新建时打开表单，标题显示为新建 Plan", () => {
      ...
      expect(screen.getByTestId("modal-title")).toHaveTextContent("新建 Plan");
    });
  });
  ...
  describe("保存时 reminder_times 包含在数据中", () => {
    it("保存时 reminder_times 包含在 PlanFormData 中", async () => {
      render(
        <PlanForm
          open={true}
          allTags={mockTags}
      ...
      const titleInput = screen.getByPlaceholderText("计划标题...");
      fireEvent.change(titleInput, { target: { value: "新计划" } });
    ...
    it("未设置提醒时间时，保存不包含 reminder_times 字段", async () => {
      ...
      const titleInput = screen.getByPlaceholderText("计划标题...");
      fireEvent.change(titleInput, { target: { value: "无提醒计划" } });
    ...
    it("reminder_times 为空数组时保存为 undefined", async () => {
      ...
      const titleInput = screen.getByPlaceholderText("计划标题...");
      fireEvent.change(titleInput, { target: { value: "测试计划" } });
  ...
  describe("编辑时显示已有的 reminder_times", () => {
    it("编辑时显示已有的提醒时间", () => {
      ...
      expect(screen.getByTestId("modal-title")).toHaveTextContent("编辑 Plan");
  ...
  describe("表单基本功能", () => {
    it("正确渲染表单", () => {
      ...
      expect(screen.getByPlaceholderText("计划标题...")).toBeInTheDocument();
  ...
  describe("日期选择", () => {
    it("可以填写开始日期和结束日期", () => {
      ...
      // 找到两个日期输入框（type="date"）
      const dateInputElements = dateInputs.filter(
        (input) => (input as HTMLInputElement).type === "date",
      );
      expect(dateInputElements).toHaveLength(2);
    });
    ...
    it("保存时包含日期信息", async () => {
      ...
      const titleInput = screen.getByPlaceholderText("计划标题...");
      fireEvent.change(titleInput, { target: { value: "带日期计划" } });
  ...
  describe("标签选择", () => {
    ...
    it("保存时包含选中的标签", async () => {
      ...
      const titleInput = screen.getByPlaceholderText("计划标题...");
      fireEvent.change(titleInput, { target: { value: "带标签计划" } });
