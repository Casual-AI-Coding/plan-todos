import type { Circulation, CirculationLog, CirculationType, PeriodicFrequency } from "@/lib/types";
import {
  getCirculations,
  getCirculation,
  getCirculationLogs,
...
const { useGetAll: useCirculations, useGetOne: useCirculation, useCreate: useCreateCirculation, useUpdate: useUpdateCirculation, useDelete: useDeleteCirculation } = createEntityHooks<Circulation, CreateCirculationInput, UpdateCirculationInput>({
...
export function useCirculationLogs(
  ...
  limit: number = 20,
  options?: Omit<UseQueryOptions<CirculationLog[], Error>, "queryKey" | "queryFn">,
) {
...
export function useCheckinCirculation(
  options?: Omit<UseMutationOptions<Circulation, Error, { id: string; note?: string; count?: number }>, "mutationFn">,
) {
  const queryClient = useQueryClient();
  return useMutation<Circulation, Error, { id: string; note?: string; count?: number }>({
    mutationFn: async ({ id, note, count }) => checkinCirculation(id, note, count),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: circulationKeys.circulation(variables.id) });
      queryClient.invalidateQueries({ queryKey: circulationKeys.circulationLogs(variables.id) });
    },
    ...options,
  });
}
...
export function useUndoCheckinCirculation(
  ...
  return useMutation<Circulation, Error, string>({
    ...
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({ queryKey: circulationKeys.circulation(id) });
      queryClient.invalidateQueries({ queryKey: circulationKeys.circulationLogs(id) });
    },
    ...options,
  });
}

export { useCirculations, useCirculation, useCreateCirculation, useUpdateCirculation, useDeleteCirculation };